...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        CGRect titleFrame = [self.titleLabel.text boundingRectWithSize:CGSizeMake(labelMaxWidth,self.titleLabel.font.lineHeight)
                                                               options:NSStringDrawingUsesLineFragmentOrigin
                                                            attributes:@{NSFontAttributeName: self.titleLabel.font}
                                                               context:nil];
        CGRect subtitleFrame = [self.subtitleLabel.text boundingRectWithSize:CGSizeMake(labelMaxWidth,self.subtitleLabel.font.lineHeight)
                                                                     options:NSStringDrawingUsesLineFragmentOrigin
                                                                  attributes:@{NSFontAttributeName: self.subtitleLabel.font}
                                                                     context:nil];
        // Method returns values that are fractional --  take the ceilf to get equivalent values
        CGSize titleSize = CGSizeMake(ceilf(titleFrame.size.width), ceilf(titleFrame.size.height));
        CGSize subtitleSize = CGSizeMake(ceilf(subtitleFrame.size.width), ceilf(subtitleFrame.size.height));
